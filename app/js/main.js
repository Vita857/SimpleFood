...
document.addEventListener('DOMContentLoaded', () => {
  ...
  burger.addEventListener('click', () => {
    mobileMenu.classList.toggle('menu__burger--active'); 
    if (mobileMenu.classList.contains('menu__burger--active'))  { 
...
$(window).on('load resize', function(){
  if ($(window).width() < 375){
    $('.restaurant__menu:not(.slick-initialized)').slick({
      arrows: false,
      dots:true,
      infinite:true,
      speed:100,
    ...
    $(".restaurant__menu.slick-initialized").slick("unslick");
